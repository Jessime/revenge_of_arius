...
class TextBox:
    ...
    def update(self, scr):
        ...
        if len(s) > 0:
            ...
            label = self.font.render(s, 5, self.color)
        self.string = ''.join(self.str_list)
        pygame.draw.rect(scr, self.color, self.rect, width)
        scr.blit(label, [self.rect[0]+14 , self.rect[1]])

# Initialize textbox
def init_textbox(x1, y1, x2, y2):
    textbox = TextBox(pygame.Rect(x1, y1, x2, y2), 1)
    return textbox
